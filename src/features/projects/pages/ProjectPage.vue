<template>
  <div>
    ...
    <div v-if="projectStore.isLoading && !projectStore.projects.length" class="text-center py-10">
      <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-pink-500"></div>
    </div>
    <div v-else class="bg-[#141f26]/40 border border-white/5 rounded-3xl overflow-hidden shadow-xl">
      <table class="w-full text-left border-collapse">
        <thead>
          <tr
            class="bg-white/5 border-b border-white/5 text-gray-400 text-xs uppercase tracking-wider"
          >
          ...
          >
            <td class="p-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-gray-400 max-w-xs truncate mt-0.5">
                    {{ project.description }}
                  </div>
                </div>
              </div>
            ...
            <td class="p-4">
              <div class="flex flex-wrap gap-1">
                <span
                  v-for="(tech, index) in (Array.isArray(project.tech_stack)
                    ? project.tech_stack
                    ...
                    .filter(Boolean)"
                  :key="index"
                  class="bg-pink-500/10 text-pink-400 text-[10px] font-medium px-2.5 py-1 rounded-full whitespace-nowrap block"
                >
                  {{ tech }}
                </span>
              </div>
        ...
        <form @submit.prevent="handleSubmit" class="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div>
            ...
            <input
              ...
              required
              class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-pink-500 transition-colors text-sm"
              placeholder="e.g. Internship, Academic..."
            />
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2"
...
const openModal = (project: Project | null = null) => {
  ...
  if (project) {
    ...
    form.tech_stack = Array.isArray(project.tech_stack)
...
const handleSubmit = async () => {
  ...
  formData.append('tech_stack', form.tech_stack.trim())
